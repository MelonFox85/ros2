 ...
 *   - Требуемое напряжение для двигателей (в вольтах).
 *
 ...
 *   - [0] V_left_requested, [1] duty_left, [2] V_right_requested, [3] duty_right, [4] emergency_stop
 */
...
private:
    ...
    {
        // <<< ИЗМЕНЕНО: QoS для команд моторам согласован с издателями (PID/LQR) >>>
        // Используем политику "Transient Local", чтобы гарантированно получить
        // последнюю отправленную команду, даже если этот узел запустился позже.
        // Это решает проблему "не крутящегося мотора" при старте.
        auto motor_cmd_qos = rclcpp::QoS(rclcpp::KeepLast(1)).transient_local();
        auto qos_sensor = rclcpp::SensorDataQoS();
        auto qos_status = rclcpp::QoS(rclcpp::KeepLast(1)); // Для статуса достаточно базового QoS
        
        imu_sub_ = create_subscription<ImuMsg>("imu/data", qos_sensor,
            std::bind(&PwmMotorController::imu_callback, this, std::placeholders::_1));
            
        left_cmd_sub_ = create_subscription<Float32Msg>("motor_cmd/left", motor_cmd_qos,
            std::bind(&PwmMotorController::left_cmd_callback, this, std::placeholders::_1));
            
        right_cmd_sub_ = create_subscription<Float32Msg>("motor_cmd/right", motor_cmd_qos,
            std::bind(&PwmMotorController::right_cmd_callback, this, std::placeholders::_1));
        
        motor_status_pub_ = create_publisher<Float32ArrayMsg>("motor_status", qos_status);
